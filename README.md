...
* Создается единообразная административная панель для всех сайтов. Если где-то нужно заменить картинку, текст, скрыть/показать блоки при moдeрaции не нужно будет ображаться к "программисту", чтобы это работало. Достаточно сделать пару кликов. Что сэкономит всем силы, время и нервы =)
2. **Преимущества для SEO**
* Высокие показатели загрузки сайта (Pagespeed будет выдавать реальные максималки);
...
* Экономия на хостинге (но это пока не точно). Ибо все данные хранятся в одном месте, поэтому нужно будет оплачивать только topdg.ru, проекты можно размещать на других специальзированных ресурсах( *этот пункт нужно будет конкретно смотреть на сколько он реально рабочий*)
...
2. Далеко не каждый разработчик сможет поддерживать с такую систему, поэтому в случае чего придется кому-то платить хорошую котлету. Ибо это технологии другого уровня и таких разработчиков как правило разбирают с руками и ногами.

Ну тут, как говорится, каждый сам решает. Вообще это введение, поэтому хватит пока. Ниже будет новые правила разработки, если эти будут одобрены.